...
from app.helpers import (Command, get_config, get_file_data, parse_args_main, storage_change_file_block,
                         storage_delete_file, write_file)

Base.metadata.create_all(bind=engine)


def main():
    (command, file_path, input_file_data, output, block_id, block_data) = parse_args_main()
    ...
    match command:
        ...
        case Command.WRITE.value:
            ...
            with SessionLocal() as session:
                write_success = write_file(session, file_path, input_file_data, host_list)
