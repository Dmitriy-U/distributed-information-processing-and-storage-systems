...
def init_db(session: Session):
    ...
    session.execute(f"CREATE TABLE IF NOT EXISTS orders (uuid UUID, product_uuid VARCHAR, amount FLOAT, fulfilled BOOLEAN, timestamp TIMESTAMP, PRIMARY KEY ((product_uuid), timestamp, uuid)) WITH CLUSTERING ORDER BY (timestamp DESC)")
...
def make_ceed_random(session: Session, ceed_number: int):
    ...
    for i in range(ceed_number):
        ...
        session.execute(f"INSERT INTO orders (uuid, product_uuid, amount, fulfilled, timestamp) VALUES (uuid(), '{product.get('uuid')}', {product.get('amount')}, false, {timestamp})")
